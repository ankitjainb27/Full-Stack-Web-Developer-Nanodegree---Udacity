...
SESSION_PARTICULAR_GET_REQUEST = endpoints.ResourceContainer(
    message_types.VoidMessage,
...
class ConferenceApi(remote.Service):
    ...
    def _createSessionObject(self, request):
        ...
        if speaker:
            ...
            q = Session.query().filter(formatted_query).count()
            print(q)
            if q > 0:
                memcache.add(MEMCACHE_SPEAKER_KEY, speaker)
    ...
    def getConferenceSessions(self, request):
        ...
        formatted_query = ndb.query.FilterNode('conferenceId', '=', request.websafeConferenceKey)
        q = Session.query().filter(ndb.GenericProperty('conferenceId') == request.websafeConferenceKey)
        q = Session.query().filter(formatted_query)
        # q = Session.query().filter('conferenceId =', request.websafeConferenceKey)
        print(q)
        print("-----")

        # return ConferenceForm
        return SessionForms(items=[self._copySessionToForm(conf) for conf in q])

    @endpoints.method(SESSION_SPEAKER_GET_REQUEST, SessionForms,
                      path='getconferencesessionsspaker',
    ...
    @endpoints.method(SESSION_SPEAKER_GET_REQUEST, SessionForms,
                      path='getsession',
                      http_method='GET', name='getSessionsfilter')
    def getSessionsfilter(self, request):

        # get Conference object from request; bail if not found
        """BadRequestError Only one inequality filter per query is supported Encountered both typeOfSession and startTime"""
        print(request.speaker)
        formatted_query = ndb.query.FilterNode('startTime', '<', 19)
        formatted_query1 = ndb.query.FilterNode('typeOfSession', '!=', ['Workshop'])
        q = Session.query().filter(formatted_query1)
    ...
    @endpoints.method(SESSION_TYPE_GET_REQUEST, SessionForms,
                      path='getconferencesessionstype/{websafeConferenceKey}',
    ...
    def getConferenceSessionsByType(self, request):
        ...
        print(request.websafeConferenceKey)
        formatted_query = ndb.query.FilterNode('conferenceId', '=', request.websafeConferenceKey)
        formatted_query1 = ndb.query.FilterNode('typeOfSession', 'in', request.typeofsession.split(","))
        # q = Session.query().filter(ndb.GenericProperty('typeOfSession IN'), ['Workshop'])
        q = Session.query().filter(formatted_query).filter(formatted_query1)
        # q = Session.query().filter('conferenceId =', request.websafeConferenceKey)
        print(q)
        print("-----")
    ...
    def _sessionRegistration(self, request, reg=True):
        ...
        if reg:
            ...
            if sess.conferenceId not in prof.conferenceKeysToAttend:
                raise ConflictException(
                    "You have to first register in the coference")
    ...
    def getSessionsInWishlist(self, request):
        ...
        sessions_list = ndb.get_multi(sess_keys)

        # return set of ConferenceForm objects per Conference
        return SessionForms(items=[self._copySessionToForm(conf) for conf in sessions_list])
    ...
    @endpoints.method(SESSION_PARTICULAR_GET_REQUEST, BooleanMessage,
